...
Recognition uses `cv::FaceRecognizerSF` (OpenCV objdetect module, already
bundled) with the official OpenCV Zoo SFace model:
- Designed to pair with YuNet (alignCrop consumes YuNet landmark rows)
- 112x112 input, 128-d embeddings, cosine matching (official threshold 0.363)
- ~38 MB model, checksum-verified at download

ONNX Runtime is no longer needed and is not bundled.
